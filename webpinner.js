...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class SpringOP {
        constructor(body, anchor, length) {
                ...
                this.anchor = anchor
                if(body.y%50 > 24){
                    this.beam = new LineOP(body, anchor, color, 2.9)
                }else{
                    this.beam = new LineOP(body, anchor, color2, 2.9)
                }
                this.length = length
            this.gravity = 0
            // this.width = width
    ...
    class Jelly{
        constructor(x,y, rip){
            ...
            for(let t = 1;t<4;t++){
                let x1 = this.x
                let y1 = this.y
                let dot = new Circle(x1, y1,5.01, "white", 0, 0, .908,1)
